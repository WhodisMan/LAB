...
int n;
struct process{
    int p,at,bt,pri,ct,tat,wt,st;
}p[MAX];

void display(){
	printf(" ");
	for(int i=0;i<7*16;i++) printf("-");
    printf("\n");
...
void sort(int code){
    ...
    if(code == 1){
        for(int i=0;i<n;i++)
            for(int j=i;j>0 && p[j-1].pri > p[j].pri;j--)
                swap(&p[j-1],&p[j]);
    }
    // process
    if(code == 2){
...
void priority(){
    ...
    for(int i=0; i<n;i++){
        ...
        // sorts the list according to PRIORITY for arival time less than completion time of the previous process.
        for(int x=1;x<n;x++){
